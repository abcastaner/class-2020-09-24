...
```{r}
ugrad_diff <- inner_join(sep_old, sep_new, by = "id") %>% 
...
```{r}
sep_joined_for_graph <- ugrad_diff %>%
  slice(1:5, 412:416) %>%
  mutate(positive = ifelse(diff > 0, "Increase", "Decrease")) %>%
  mutate(ugrad_diff_abs = abs(diff)) %>%
  group_by(positive) %>%
  ggplot(aes(name.x, abs(diff))) +
  geom_col() +
  facet_wrap(~positive, scales = "free") +
  coord_flip() +
  theme_bw()

sep_joined_for_graph
